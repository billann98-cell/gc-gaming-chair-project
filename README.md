...
**時間軸**
- 右上角可切換**日／週／月**刻度。日刻度一天一欄（分隔線只畫在每週一，否則會變成一片格子）；週刻度上排顯示月份、下排顯示每週起始日；月刻度上排顯示年、下排顯示月份

**國定假日（多本日曆）**

`calendars/holidays.json` 可放多本日曆，每本自己一個顏色，例如台灣與大陸分開。因為兩邊放假影響的是不同的人：台灣假日停的是設計與驗證，大陸假日停的是工廠。

- 甘特圖用直向色帶標出假日，週末是灰色（自動計算，不必寫進資料）
- 同一天兩邊都放假時，兩條色條會**分上下車道顯示**，看得出是哪兩邊
- 工期徽章顯示「日曆天」後面接**各本日曆各自的可工作天數**，用對應顏色標示；滑到長條上會列出各邊分別扣掉哪些假日
- 圖表下方的「假日標示說明」會把每本日曆實際標記的日期全部列出來，並把不確定的項目標成「待核對」
- `kind` 可填 `holiday`（放假）或 `workday`（大陸調休把週末改成上班日）

**注意**：台灣補假由行政院人事行政總處公告、大陸放假與調休由國務院辦公廳公告，通常前一年底才定案。資料裡標 `verify` 的項目請自行核對後把該欄刪掉。
